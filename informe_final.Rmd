...
correl <- datos %>%
  select(-c(country, othermonetary, otherbop, CESI_INDEX)) %>%
  rename(#Cambiamos los nombres acá para que la matriz de correlaciones quepa en el ancho del documento
    hospbed="hospitalbed", 
    totcases="totalcases"
  ) %>% 
cor()
...
plot.PCA(acp3, axes= c(1,2), choix = c("var"), graph.type = "ggplot") +
  ggtitle(NULL) +
...
El procedimiento del ACP desde esta perspectiva resulta simétrico al de la de la de la Nube de Columnas. Se proyecta el conjunto de observaciones en el subespacio $W^*$ de dimensión $k \le J$, siendo $J$ el número de variables, de manera de minimizar la inercia del subespacio. Desde esta perspectiva la métrica utilizada es la matriz $M$ y se emplea la matriz de pesos $D$, empleada como matriz de pesos y métrica respectivamente al proyectar la nube de variables.   
...
mat_dist_cos2 %>% as.data.frame %>%  arrange(desc(norma)) %>% round(2) %>% 
  head(10) %>% 
  xtable("Cosenos de los ángulos al plano, y normas, para los 10 individuos con mayor norma.")


```
Revisar esto porque creo que el ángulo de la obs 130 quedó desactualizado.
...
En la tabla anterior se presentan los valores de la suma de los cosenos al cuadrado de los ángulos que forman las observaciones con los dos primeros ejes y la norma respecto al baricentro, ordenando las observaciones decrecientemente según esta última. Se aprecia que la observación correspondiente a San Marino tiene la mayor norma (9,7) y una suma de cosenos al cuadrado de 0,67, se debe tener en cuenta que podría estar afectando el análisis teniendo en cuenta que su calidad de representación en el primer plano no es alta y su norma es particularmente alta. Efectuar el ACP considerando la observación influyente como un dato suplementario representa una ganancia en inercia explicada en el plano del 4% aproximadamente, por lo que esto puede ser viable para tener una mejor descripción de las variables/individuos en las dimensiones elegidas. El resto de observaciones con norma alta cuentan con valores de la suma de cosenos al cuadrado superiores a 0,5 y en general mayores a 0,7 por lo que a priori no se pensarían influyentes.
...
## Anexo 1: gráficos para el análisis de observacoines atípicas
...
```{r, include=TRUE, fig.cap="Gráfico de caja para el gasto en salus de los países."}
