...
En la tabla anterior podemos ver que la primer componente principal logra explicar un 53.32% de la variabilidad de la nube de puntos, y la segunda componente principal un 19.45%. Esto significa que con el primer plano principal logramos explicar prácticamente un 72.77% de la variabilidad total, una cantidad muy importante a nuestro juicio dado que reducimos la cantidad de dimensiones a dos, siendo 5 el número original de variables en la matriz de datos.
...
En el grafico anterior podemos ver representados los vectores asociados a las variables proyectadas en el primer plano principal, teniendo en cuenta que el largo del vector en relación al círculo unitario indica qué tan bien está representada esa variable en el primer plano principal. Podemos ver que en general los vectores tienen una longitud relativamente cercana a la unidad, destacándose las variables *medage*, *hospitalbed*, *noinfectionrate* y *gdppercap*. A excepción de *healthexp*, notamos que todas las variables "necesitan" de ambos ejes para poder estar bien representadas.

Por otro lado, vemos que todas las variables apuntan en la misma dirección del primer componente principal a excepción de *noinfectionrate* que apunta en sentido contrario. Esto quiere decir que un aumento en *noinfectionrate* se expresa con una disminución del valor de la primera componente, mientras que un aumento en cualquiera de las demás variables se expresa con un aumento en dicha componente.
...
Se aprecia que *medage* y *gdppercap* tienen correlaciones similares con la componente 2, aunque una con signo positivo y la otra con signo negativo. Esto es fácil de ver en el gráfico, dado que los dos vectores tienen longitud similares y ángulos opuestos respecto a este eje.  También se ve que la componente 2 es en el primer plano principal la que contribuye en mayor medida a explicar la variable *noinfectionrate*, puesto que la correlación es mayor (0,76). Por otro lado las correlaciones de las demás variables con esta componente son en general más bajas que con la componente 1, lo cual significa que no representa esas variables tan bien como el otro eje.
...
```
Por otro lado, calculamos las correlaciones del índice de estímulo económico utilizado como variable suplementaria con los dos ejes principales, siendo la correlación con el primer eje de `r coord.ind.1` y con el segundo eje de `r coord.ind.2`. Esto significa que el eje construido por las variables *medage*, *gdppercap*, *healthexp* y *hospitalbed* se comporta de manera relativamente similar al índice de estímulo económico; no así con el eje construido principalmente a partir de las variables *noinfectionrate* y *hospitalbed*. 
