...
```{r graficos de silueta, out.width= "100%", include= TRUE }
...
Analizando los gráficos de siluetas y los valores presentados en la Figura xx se aprecia que para ambas estructuras de grupo la silueta promedio de todas las observaciones toma un valor de 0.34 aproximadamente. Sin embargo observándose la silueta promedio dentro de cada uno de los grupos en la estructura de dos grupos la silueta promedio de uno de ellos es negativa y próxima a 0, lo que indica que en promedio la distancia de las observaciones a las demás de su mismo grupo es casi igual a las distancia de las observaciones a las del otro grupo. Esto es consecuencia del elevado número de observaciones con silueta negativa en el grupo 2, aproximadamente la mitad, las cuales son más similares a las observaciones del grupo 1, lo cual no es deseado para hacer una buena clasificación. Para la estructura de 4 grupos si bien el promedio global de las siluetas es igual al de la otra estructura vemos que ninguno de los grupos tiene siluetas promedio negativas y la menor es de 0.11. Esto indicaría que las observaciones están mejor asignadas en sus grupos al considerar cuatro de ellos.
...
En la Figura xx se puede observar el gráfico de los países proyectados en el primer plano principal, conformado por las dos componentes principales empleadas en la formación de los clusters. Lo primero que se puede observar es que el grupo 4 es el que más se aleja de los demás, dado que las observaciones que lo componen tomaron valores altos de las dos variables consideradas (que a su vez considerando el conjunto de las variables con las que se formaron las componentes, tomaban valores atípicos, en particular en la variable *infectionrate*). Los países del grupo 1 son los que toman valores más bajos en la dimensión 1, que de acuerdo a la caracterización de los ejes realizada en *Marsella y Saldaña (2021)* corresponde principalmente a valores más bajos de variables de desarrollo económico. En cuanto a la dimensión 2 toman valores próximos a 0 y siguiendo la caracterización de este eje que principalmente se daba por la tasa de infección, esto indicaría que los niveles de infección de los países de este grupo están más cercanos al valor medio de está variable. Los países de los grupos 2 y 3 se ven diferenciados principalmente por la primera dimensión, donde los del grupo 3 tienden a tener mayores valores de esta variable, lo que se puede interpretar como un mayor nivel de desarrollo económico. También hay una tendencia a que los del grupo 2 tienen mayores valores de la segunda dimensión lo que se podría interpretar como que los países de este cluster tienen menores tasas de infección en general. 



```{r boxplot noinfectionrate}
datos_disc_multinom <- as.data.frame(cbind(datos[,vars_cluster], graf2[,-3]))

ggplot(datos_disc_multinom) + geom_boxplot(aes(x = as.factor(grupos_acp2), y = noinfectionrate))

```


```{r}
ggplot(datos_disc_multinom) + geom_boxplot(aes(x = as.factor(grupos_acp2), y = gdppercap))
